...
class AKKIIExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle, ShouldAutoSize
{
    protected $startDate;
    protected $endDate;
    protected $title;
     ...
     * @return array
     */
    public function map($akkii): array
    {
        // Jika ada item, ambil item pertama untuk ditampilkan
        $item = $akkii->items->first();
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        if ($dataCount > 0) {
            // Hitung total baris dengan mempertimbangkan item untuk setiap AKKII
            $totalRows = 3; // Header dimulai dari baris ke-3
            foreach ($this->collection() as $akkii) {
                $itemCount = $akkii->items->count();
                $totalRows += ($itemCount > 0) ? $itemCount : 1;
            }
            
            $sheet->getStyle('A3:R' . $totalRows)->applyFromArray($styleArray);
        } else {
